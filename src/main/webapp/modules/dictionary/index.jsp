...
function loadData(pageNo){	
	$.ajax({
	    type: "POST",
	    url: "<p:basePath/>pc/getDictionaryPC.act",
	    data : "pageNo="+pageNo,
	    dataType: "html",
...
function _serLast(){
 ...
 //新增
 function demo_add() {
	 window.location.href="<p:basePath/>pc/addDictonary.act?";
 }
 //修改
 function demo_edit(obj) {
 	 window.location.href="<p:basePath/>pc/editDictonary.act?dicCode="+obj;
 }
 //删除
 function demo_delete(dicId) {
 	if(confirm("确定要删除吗?")){
    	if(confirm("请再次确认！确定要删除?")){
    		self.location.href="<p:basePath/>pc/deleteDictonary.act?dicId="+dicId;
    	}
    } 
 }
