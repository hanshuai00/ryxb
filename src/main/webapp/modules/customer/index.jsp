...
<div class="red_content" id="rechargeBody">
	<!-- 充值页面 -->
	<%@ include file="addRecharge.jsp"%> 
</div>
...
function loadData(pageNo){	
	$.ajax({
	    type: "POST",
	    url: "<p:basePath/>pc/getCustomerListPC.act",
	    data:{"pageNo":pageNo,
	    	"userLevel":$("#s_userLevel").val(),
...
//进充值页面
function demo_recharge(objectId) { 
	 $.ajax({
		    type: "POST",
		    url: "<p:basePath/>pc/addRechargePC.act",
...
function demo_delete(objectId) {
	if(confirm("确定要删除该用户吗?")){
   		$.ajax({
			type: "POST",
			url: "<p:basePath/>pc/deleteCustomerPC.act",
			dataType : "json",
			async: false,
	        cache: false,
			data : {"objectId" :objectId},
...
function getSmsCode(){
	$.ajax({
        type: "POST",
        url:"<p:basePath/>pc/getRechargeSmsCode.act",
        data:$('#rechargeForm').serialize(),// 你的formid
        async: false,
        cache: false,
        success : function(resp) {
			resp = eval('(' + resp + ')');
...
//充值页面——保存
function batch_save(){
    if($("#actualAmount").val()==null ||$("#actualAmount").val()=="" ){
		$("#message").html("充值金额不能为空!");
		return false;
	}
    if($("#actualAmount").val()<0){
    	$("#message").html("充值金额不能小于0!");
    	return false;
    }
    if($("#actualAmount").val().length>16){
    	$("#message").html("充值金额长度不能大于16!");
    	return false;
    }
	...
	$.ajax({
        type: "POST",
        url:"<p:basePath/>pc/saveRechargePC.act",
        data:$('#rechargeForm').serialize(),// 你的formid
        async: false,
        cache: false,
        success : function(resp) {
